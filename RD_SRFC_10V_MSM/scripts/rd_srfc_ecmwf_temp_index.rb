...
def check_index(lvl1,index,lvldata,asmid,ft)
  ...
  upper = []
  start = false
  $config["mk2_pressure_level_ecmwf"].each{|lvl|
    if lvl == lvl1
      start = true
    end
...
def get_temp_index(rawdata)
  ...
  rawdata.each_pair{|asmid,ftdata|
    ...
    ftdata.each_pair{|ft,lvldata|
      ...
      level1 = nil
      # 低→高
      $config["mk2_pressure_level_ecmwf"].each_index{|i|
        level1 = $config["mk2_pressure_level_ecmwf"][i]
        if lvldata[level1]["GeopotentialHeight"] == alt_data["ALT"]
          ...
          level0 = $config["mk2_pressure_level_ecmwf"][i-1]
          # 標高差（高-低）
      ...
      if indexdata[asmid][ft]["Temperature"] == nil
        # データ異常
        $log.write("%s ft=%s invalid input data." % [ asmid, ft ])
        return nil
      else
...
def get_onebase_data(mkConn,btime,inputtable,point_list)
  ...
  start_time = Time.now
  end_time = start_time + 3600 * 96
  # 面データから地点切り出し
  rawdata = {}
  ft_list = mkConn.get_ft_list(inputtable, btime, "Temperature")
  ft_list.each{|ft|
    if btime + ft < start_time || btime + ft > end_time
      next
    end
    $log.write("read ft=%s" % [(btime + ft).to_s])
    param_list = []
    $config["mk2_pressure_level_ecmwf"].each{|level|
    ...
    point_list.each_index{|i|
      ...
      param_list.each{|param|
        ...
        if isinvalid(elm_data[i])
          $log.write("%s %s %s Temperature=%s" % [ pid,ft,param.level,elm_data[i] ])
          $log.write("ecwmf data import not completed.")
          return nil
        else
  ...
  if $spool
    ...
    savedir = $config["spool_dir"] + $config["ecmwf_raw_temp_index"]
...
def get_mk2_latesttime(mkConn,inputtable)
  latesttime = Time.now
  starttime = latesttime - 3600 * 24 * 3
  time_list = mkConn.get_time_list(inputtable,starttime,latesttime)
  if time_list.size < 1
    $log.write("no data in mk2 table.")
    return nil
  end
  latesttime = time_list.last
  ft_list = mkConn.get_ft_list(inputtable, latesttime, "Temperature")
  $log.write("btime=%s Temperature ft count=%s" % [latesttime.to_s,ft_list.size])
  if ft_list.size < 125
    $log.write("ecwmf data import not completed.")
    return nil
  end
  ft_list = mkConn.get_ft_list(inputtable, latesttime, "GeopotentialHeight")
  $log.write("btime=%s GeopotentialHeight ft count=%s" % [latesttime.to_s,ft_list.size])
  if ft_list.size < 125
    $log.write("ecwmf data import not completed.")
    return nil
  end
  $log.write("latest_time=%s" % [latesttime.to_s])
  return latesttime
end
...
begin
  ...
  inputtable = $config["mk2_ecmwf_table"]
  # 最新時刻を取得
  latesttime = nil
  if debugtime != nil && debugtime.size == 10
    ...
    $log.write("ecmwf debugtime=%s" % [latesttime.to_s])
  else
    latesttime = get_mk2_latesttime(mkConn,inputtable)
  end
  if latesttime == nil
    exit
  end
