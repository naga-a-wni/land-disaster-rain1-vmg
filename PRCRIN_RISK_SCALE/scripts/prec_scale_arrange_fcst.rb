...
# [pointid][FT]["keep_count_2"] = keep_count_2
$scale_ft_prev = nil
...
#    print "path 0\n"
    if ref_ft["INDEX_rain_inner"] > 0
      ...
      # 継続開始
      return false,1,index_type,prev_exist
    else
      # 今回スケールなし
      return false,0,0,prev_exist
    end
  end
  # 前回スケールあり
...
#      print "path 2\n"
      ...
      if keep_count < 1
        ...
        # 継続続行
        return false,keep_count + 1,index_type,prev_exist
      end
    else
...
#    print "path 4\n"
    ...
    if prv_scale == ref_ft["INDEX_rain_inner"]
      # 今回スケールが前回スケールと同じ
      return true,keep_count,index_type,prev_exist
    else
      # 今回スケールが前回スケールより下がる
      # 継続リセット
      return true,1,index_type,prev_exist
    end
  end
end
...
def get_mk2_scale_fcst(mkConn, announcetime)
  ...
  element_list = [ 'INDEX_rain:INT8','keep_count:INT32','index_type:INT8','keep_count_2:INT32' ]
  ...
  params.each{|pm|
    ...
    keep_count_2 = pd.get_data(pm, 'keep_count_2')
    $point_id.each_index{|i|
      ...
      $scale_ft_prev[pointid][pm.ft]["keep_count_2"] = keep_count_2[i]
    }
  }
#  p $scale_ft_prev
...
#  p new_ft_scale
  ...
  # new_ft_scale[FT][pointid]["keep_count_2"]
  new_ft_scale.each_key{|ft|
    ...
    $point_id.each{|pid|
      if new_ft_scale[ft][pid] != nil
        ...
        keep_count_2.push(new_ft_scale[ft][pid]["keep_count_2"])
      else
    ...
    pd.set_data(param, "keep_count_2:INT32", keep_count_2)
  }
  ...
  for i in 0...customer_count
    ...
    for j in 0...area_count
      ...
      for k in stratft...fcst_count
        ...
        new_ft_scale[k][pointid]["keep_count_2"] = -1
...
#        print "FT%d before keep_60min scale=%s\n" % [k,ref["customer_data"][i]["area_data"][j]["INDEX"][k]["INDEX_rain_inner"]]
        reset, new_ft_scale[k][pointid]["keep_count"], new_ft_scale[k][pointid]["index_type"],prev_exist = keep_60min_fcst(pointid, k, ref["customer_data"][i]["area_data"][j]["INDEX"][k])
...
#        print "FT%d output scale=%s\n" % [k,ref["customer_data"][i]["area_data"][j]["INDEX"][k]["INDEX_rain_inner"]]
        # FT3のみ
        if k == 3 && reset == false && prev_exist
          # ---------------------
          # 20分継続処理 Ver. 2.1
          # ---------------------
          # 生スケールが時間雨量、もしくは組み合わせ雨量による条件でレベル1以上になっており、
          # かつ10分前に出力されたスケールからレベルが上がる場合、
          # 前10分に出力されたスケールを20分間維持する。
          # さらに10分後は生スケールが出力スケールとして反映される。
          new_ft_scale[k][pointid]["keep_count_2"] = 0
          if $scale_ft_prev[pointid][k]["keep_count_2"] == 1
#            print "%s ----- 1 -----\n" % [Time.now.to_s]
            # 継続2回目
            ref["customer_data"][i]["area_data"][j]["INDEX"][k]["INDEX_rain_inner"] = $scale_ft_prev[pointid][k]["out"]
            new_ft_scale[k][pointid]["out"] = $scale_ft_prev[pointid][k]["out"]
            new_ft_scale[k][pointid]["keep_count_2"] = 2
            new_ft_scale[k][pointid]["keep_count"] = 0
          elsif $scale_ft_prev[pointid][k]["keep_count_2"] == 2
#            print "%s ----- 2 -----\n" % [Time.now.to_s]
            # 継続終了
            ref["customer_data"][i]["area_data"][j]["INDEX"][k]["INDEX_rain_inner"] = index_rain_raw
            new_ft_scale[k][pointid]["out"] = index_rain_raw
            new_ft_scale[k][pointid]["keep_count"] = 0
            # 条件更新
            if ref["customer_data"][i]["area_data"][j]["INDEX"][k]["INDEX_rain_name_inner"].index("3hour") != nil
              new_ft_scale[k][pointid]["index_type"] = 1
            elsif ref["customer_data"][i]["area_data"][j]["INDEX"][k]["INDEX_rain_name_inner"].index("24hour") != nil
              new_ft_scale[k][pointid]["index_type"] = 2
            else
              new_ft_scale[k][pointid]["index_type"] = 0
            end
          else
            if index_rain_raw > 0 && new_ft_scale[k][pointid]["index_type"] <= 0
              if $scale_ft_prev[pointid][k]["out"] < index_rain_raw
#                print "%s ----- 0 -----\n" % [Time.now.to_s]
                # 継続初回
                ref["customer_data"][i]["area_data"][j]["INDEX"][k]["INDEX_rain_inner"] = $scale_ft_prev[pointid][k]["out"]
                new_ft_scale[k][pointid]["out"] = $scale_ft_prev[pointid][k]["out"]
