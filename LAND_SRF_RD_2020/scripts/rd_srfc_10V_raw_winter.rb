...
#      fix_rdtmp_fts = []
      for j in 0...fcst_count
        ...
        # C3OP 全部float
        # [zone_id][ft][sopno]["SNWFLL_1HOUR_TOTAL"]
        #                     ["PRCRIN_1HOUR_TOTAL"]
        #                     ["AIRTMP"]
        #                     ["WX"]
        if $change_data != nil && $change_data[zone_id] != nil && $change_data[zone_id][ft] != nil
          $change_data[zone_id][ft].each_pair{|sop,cdata|
            cdata.each_pair{|elm,value|
              $log.write("c3op change_data %s %s %s %s=%s" % [zone_id,ft.to_s,sop,elm,value])
              if elm == ""
                refr["point_data"][i]["FCST"][j]["SNWFLL_1HOUR_TOTAL"] = value # 時間降雪量
              else
                refw["ZONE_data"][zone_area_count]["FCAS"][j][elm] = value
              end
            }
          }
        end
...
def main()
  ...
  if $c3op
    if input_tagid != "411024527"
      $log.write("c3op input_tagid=%s not supported." % [input_tagid])
      return
    end
  end
  ...
  # c3opスプールデータ読み出し
  if $c3op
    $log.write("c3op version.")
    spool_fname = $config["c3op_change_spool"]
